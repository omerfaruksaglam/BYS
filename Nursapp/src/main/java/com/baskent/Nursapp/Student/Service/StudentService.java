package com.baskent.Nursapp.Student.Service;
...
public class StudentService {
    ...
    @Autowired
    private final PerformsRepository performsRepository;
    ...
    public SubskillStepsForStatistics getSubSkillStepsForStatistics(Long subskillid, Long studentid, Long bolumid, Long classlevel, Integer baserole) {
        ...
        //ss.removeIf(subs -> subs.getIsHead() == true);
        sss = new ArrayList<>(ss);
    ...
    public void saveGrades(SavePerforms saveGrades) {
        StringBuilder actionText = new StringBuilder("[" + saveGrades.getNewGrades().get(0).getStudentId().toString()+ ", " + saveGrades.getNewGrades().get(0).getBolumId().toString() + "]" + " userId ve bolumId ye sahip öğrenci kendisine, ["+ saveGrades.getNewGrades().get(0).getSinif().toString() + "]. sınıfı için, ");
        List<Long> yeniSS = new ArrayList<>();
        List<Long> updateSS = new ArrayList<>();
        for (PerformsDTO grade : saveGrades.getNewGrades()) {
            boolean exists = performsRepository.existsByCustomId(grade.getStudentId(), grade.getBolumId(), grade.getSubSkillStepsId(), grade.getSinif()); // Örneğin, graderRole için sabit bir değer kullanıyorsunuz gibi görünüyor.
            if (exists) {
                ...
                performsRepository.updateGrade(grade.getStudentId(),grade.getBolumId(),grade.getSubSkillStepsId(),grade.getSinif(),grade.getPerformGrade(), ZonedDateTime.now());
            } else {
                yeniSS.add(grade.getSubSkillStepsId());
                // Kayıt yok, yeni kaydı ekle. localDateTime.atZone(ZoneId.systemDefault()).toInstant().toEpochMilli()
                performsRepository.save(Performs.builder().studentId(grade.getStudentId()).bolumId(grade.getBolumId()).subskillStepsId(grade.getSubSkillStepsId()).performDate(ZonedDateTime.now()).classLevel(grade.getSinif()).grade(grade.getPerformGrade()).build()); // Grades nesnesini DTO'dan oluşturun.
            }
        }
        ...
        action_recordRepository.save(Action_Records.builder().actionTime(Instant.now()).ipAddress(clientIpBean.getClientIp()).userid(saveGrades.getNewGrades().get(0).getStudentId()).bolumId(saveGrades.getNewGrades().get(0).getBolumId()).actionText(actionText.toString()).build());
