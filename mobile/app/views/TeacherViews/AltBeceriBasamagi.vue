<template>
    <Page class="page" @navigatedTo="onNavigatedTo">
        <ActionBar :title="this.subSkill.name" class="action-bar" flat/>
        <GridLayout
            :rows="equipments !== null && equipments.length > 0 ? 'auto, 5*, auto, 26*, auto' : 'auto ,auto, auto'">

            <Label text="Malzeme Listesi:" row="0" class="equipment-list-title"
                v-if="equipments !== null && equipments.length > 0" />
                    ...
                    <StackLayout v-for="item in subSkillSteps" :key="item.subSkillStepsId" class="list-group-item">
                        <GridLayout
                            ...
                            <Label :text="item.teacherGrade === null ? '-' : item.teacherGrade.toString()"
                                ...
                                @tap="notVer && $store.state.user.userRole === 0 && !item.isHead ? pickGrade(item, 'teacherGrade', item.teacherGrade) : !item.isHead && item.teacherGrade !== null ? findGraderAndShow(item, 0) : null"
                                v-if="!item.isHead" />
                            <!-- nurseGrade -->
                            <StackLayout
                            ...
                            <Label :text="item.nurseGrade === null ? '-' : item.nurseGrade.toString()"
                                ...
                                @tap="notVer && $store.state.user.userRole === 2 && !item.isHead ? pickGrade(item, 'nurseGrade', item.nurseGrade) : !item.isHead && item.nurseGrade !== null ? findGraderAndShow(item, 2) : null" />
                        </GridLayout>
        ...
        */
        ...
        updateChangedGrades(item, gradeType, value) {
            ...
            if (existing) {
                ...
                else if (gradeType === 'teacherGrade' || gradeType === 'nurseGrade') {
                ...
                else if ((gradeType === 'teacherGrade' && parseInt(value, 10) !== item.teacherGrade) || (gradeType === 'nurseGrade' && parseInt(value, 10) !== item.nurseGrade)) {
                    const newItem = { subSkillStepsId: item.subskillStepsId, grade: parseInt(value, 10), studentId: this.student.userId, bolumId: this.$store.state.user.bolumId, sinif: this.sinif, graderId: this.$store.state.user.userId };
                    this.changedGrades.push(newItem);
                }
            }
        ...
        async subSkillBasamaklariniGetir() {
            if (this.$store.state.user.userRole === 0) {
                if (this.fromStatistic === false) {
                    await fetch(`http://${this.$store.state.user.hostIp}:8082/nursapp/app/teacher/getsubskillsteps/${this.subSkill.subskillId}/${this.student.userId}/${this.$store.state.user.bolumId}/${this.sinif}`, {
                        method: "GET",
                        headers: {
                            "Content-Type": "application/json",
                            "Authorization": "Bearer " + this.$store.state.user.token
                    ...
                    await fetch(`http://${this.$store.state.user.hostIp}:8082/nursapp/app/teacher/getsubskillstepsforstatistics/${this.subSkill.subskillId}/${this.student.userId}/${this.$store.state.user.bolumId}/${this.sinif}`, {
                        method: "GET",
                        headers: {
                            "Content-Type": "application/json",
                            "Authorization": "Bearer " + this.$store.state.user.token
                ...
                if (this.fromStatistic === false) {
                    ...
                    await fetch(`http://${this.$store.state.user.hostIp}:8082/nursapp/app/student/getsubskillstepsforstatistics/${this.subSkill.subskillId}/${this.student.userId}/${this.$store.state.user.bolumId}/${this.sinif}`, {
                        method: "GET",
                        headers: {
                            "Content-Type": "application/json",
                            "Authorization": "Bearer " + this.$store.state.user.token
                ...
                if (this.fromStatistic === false) {
                    await fetch(`http://${this.$store.state.user.hostIp}:8082/nursapp/app/secondgrader/getsubskillsteps/${this.subSkill.subskillId}/${this.student.userId}/${this.$store.state.user.bolumId}/${this.sinif}`, {
                        method: "GET",
                        headers: {
                            "Content-Type": "application/json",
                            "Authorization": "Bearer " + this.$store.state.user.token
                    ...
                    await fetch(`http://${this.$store.state.user.hostIp}:8082/nursapp/app/secondgrader/getsubskillstepsforstatistics/${this.subSkill.subskillId}/${this.student.userId}/${this.$store.state.user.bolumId}/${this.sinif}`, {
                        method: "GET",
                        headers: {
                            "Content-Type": "application/json",
                            "Authorization": "Bearer " + this.$store.state.user.token
