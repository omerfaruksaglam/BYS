...
<label class="label">İkincil Puanlandırıcı numarası</label>
<TextField  class="input-field" v-model="nurseNumber" @blur="onNurseNumberBlur"/>
...
import TeacherAnaekran from '../TeacherViews/TeacherAnaekran.vue';
...
export default {
  ...
  .catch(error => {
    ...
    const options = {
      ...
      actions: this.universityData.map(u => u.uni_name),
    };

    action(options).then((result) => {
      if (result !== "İptal") {
        this.selectedUniversity = result;
        const selectedUni = this.universityData.find(u => u.uni_name == result);
        this.selectedUniversityId = selectedUni ? selectedUni.uni_id : null;
  ...
  goBack() {
    ...
    .then(data => {
      if(data != null){
        ...
        this.$navigateTo(TeacherAnaekran,{
          clearHistory: true,
