<template>
    <Page class="page" @navigatedTo="onNavigatedTo">
        <ActionBar :title="this.skill.namee" class="action-bar" />
            ...
            <StackLayout class="list-container">
                ...
                <StackLayout v-for="item in subSkills" :key="item.subskillId" class="list-group-item">
                    <GridLayout :columns="'5*,' + ( bolumInfo.ogrenciNotVerebilirmi ? ' 1, *,' : '') + '1, *,' + (secondGraderNotVeriyormu ? ' 1, *' : '')" rows="auto">
                        ...
                        <!-- Scores -->
                        <Label :text="item.studentGrade === null ? '-':item.studentGrade.toString()" col="2" class="list-group-item-score" v-if="bolumInfo.ogrenciNotVerebilirmi"/>
                        <StackLayout :col="bolumInfo.ogrenciNotVerebilirmi ? '3':'1'" class="divider" />

                        <Label :text="item.teacherGrade === null ? '-':item.teacherGrade.toString()" :col="bolumInfo.ogrenciNotVerebilirmi ? '4' : '2'" class="list-group-item-score" />
                        ...
                        <Label :text="item.nurseGrade === null ? '-':item.nurseGrade.toString()"  :col="bolumInfo.ogrenciNotVerebilirmi ? (secondGraderNotVeriyormu ? '6' : '-1') : (secondGraderNotVeriyormu ? '4' : '-1')" class="list-group-item-score" v-if="secondGraderNotVeriyormu"/>
                    </GridLayout>
                    <!-- Horizontal divider -->
                    <StackLayout class="horizontal-divider" />
                </StackLayout>
...
<script scoped>
import StudentSubSkillSteps from './StudentSubSkillSteps.vue';
export default {
    ...
    methods: {
        goSubSkillSteps(subSkill) {
            this.$navigateTo(StudentSubSkillSteps, {
