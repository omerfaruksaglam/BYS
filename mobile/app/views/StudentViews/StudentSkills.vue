<template>
    <Page class="page" @navigatedTo="onNavigatedTo">
        <ActionBar title="Beceri seçin" class="action-bar" />
        <ScrollView>
            <StackLayout>
...
<script scoped>
import StudentSubSkills from './StudentSubSkills.vue';
import StatisticicinAltBeceri from '../TeacherViews/StatisticicinAltBeceri.vue';
export default {
    ...
    methods: {
        goSubSkill(skill) { //this.$store.state.user.userRole === 1 && !bolumInfo.ogrenciNotVerebilirmi ? false : this.notVer
            if(this.fromStatistic){
                ...
                this.$navigateTo(StudentSubSkills, {
