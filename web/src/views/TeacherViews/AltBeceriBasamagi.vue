            ...
            <table>
                ...
                <tbody>
                    <tr v-for="subskillstep in subskillSteps" :key="subskillstep.subskillStepsId">
                        ...
                        <td
                            v-if="bolumInfo.ogrenciNotVerebilirmi" @click="!subskillstep.isHead && subskillstep.performGrade !== null ? findGraderAndShow(subskillstep, 1) : null">
                        ...
                        <td v-if="this.$store.state.user.userRole !== 0 || subskillstep.isHead || !this.notVer" @click="!subskillstep.isHead && subskillstep.teacherGrade !== null ? findGraderAndShow(subskillstep, 0) : null">{{
                            subskillstep.teacherGrade === null ? '-' : subskillstep.teacherGrade }}</td>
                        <td v-if="this.$store.state.user.userRole === 0 && !subskillstep.isHead && this.notVer">
                            <!-- Dropdown buraya eklenecek -->
                            <select v-model="subskillstep.teacherGrade" @change="updateChangedGrades(subskillstep,'teacherGrade')">
                                <option class="custom-select" :value="null" disabled selected
                                    v-if="subskillstep.teacherGrade === null">-</option>
                        ...
                        <td v-if="secondGraderNotVeriyormu && ((( !this.notVer) || this.$store.state.user.userRole !== 2 ) || subskillstep.isHead)" @click="!subskillstep.isHead && subskillstep.nurseGrade !== null ? findGraderAndShow(subskillstep, 2) : null">{{ subskillstep.nurseGrade === null ? '-' :
                            subskillstep.nurseGrade }}</td>

                        <td v-else-if="secondGraderNotVeriyormu && !subskillstep.isHead && this.notVer && this.$store.state.user.userRole === 2">
                            <!-- Dropdown buraya eklenecek -->
                            <select v-model="subskillstep.nurseGrade" @change="updateChangedGrades(subskillstep, 'nurseGrade')">
                                <option class="custom-select" :value="null" disabled selected
                                    v-if="subskillstep.teacherGrade === null">-</option>
                                <option class="custom-select" :value=0>0</option>
                                <option class="custom-select" :value=1>1</option>
...
export default {
   async mounted() {
    ...
    Swal.fire({
                title: 'Puan Bilgisi',
                text: 'Sınıf seçimi başarılı bir şekilde gerçekleştirildi.',
                icon: 'success',
  ...
  return JSON.parse(bytes.toString(cryptoJs.enc.Utf8))
        ...
        async kaydet() {
            if (this.$store.state.user.userRole === 0 && this.changedGrades.length > 0) {
                await fetch(`http://localhost:8082/nursapp/app/teacher/savegrades`, {
                    method: "POST",
                    headers: {
                        "Content-Type": "application/json",
                        "Authorization": "Bearer " + this.$store.state.user.token
                ...
                await fetch(`http://localhost:8082/nursapp/app/secondgrader/savegrades`, {
                    method: "POST",
                    headers: {
                        "Content-Type": "application/json",
                        "Authorization": "Bearer " + this.$store.state.user.token
        ...
        updateChangedGrades(item, gradeType) {
            ...
            if (existing !== undefined) {
                 if (gradeType === 'teacherGrade') {
                    existing.teacherGrade = parseInt(item.teacherGrade,10);

                }else if(gradeType === 'nurseGrade'){
                    existing.nurseGrade = parseInt(item.nurseGrade,10);
                ...
                console.log(value);
                 ...
                 if ((gradeType === 'teacherGrade' && value.teacherGrade !== parseInt(item.teacherGrade, 10)) || (gradeType === 'nurseGrade' && value.nurseGrade !== parseInt(item.nurseGrade, 10))) {
                    const newItem = { subSkillStepsId: item.subskillStepsId, grade: gradeType === 'teacherGrade'? parseInt(item.teacherGrade,10):parseInt(item.nurseGrade, 10), studentId: this.student.userId, bolumId: this.$store.state.user.bolumId, sinif: this.year, graderId: this.$store.state.user.userId };
        ...
        async subSkilleriGetir() {
            if (this.$store.state.user.userRole === 0) {
                await fetch(`http://localhost:8082/nursapp/app/teacher/getsubskillsteps/${this.subskill.subskillId}/${this.student.userId}/${this.$store.state.user.bolumId}/${this.year}`,
                    {
                        method: "GET",
                        headers: {
                            "Content-Type": "application/json",
                ...
                await fetch(`http://localhost:8082/nursapp/app/secondgrader/getsubskillsteps/${this.subskill.subskillId}/${this.student.userId}/${this.$store.state.user.bolumId}/${this.year}`,
                    {
                        method: "GET",
                        headers: {
                            "Content-Type": "application/json",
